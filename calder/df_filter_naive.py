...
def rolling_time_median(jd, mag, days=300.0, min_points=10, min_days=30.0, past_only=False):
    """
    Rolling median in time. If past_only=True, uses [t0 - days, t0] (one-sided) to avoid
    future-leakage into ongoing dips. Halves 'days' down to min_days until >= min_points exist.
    """
...
def rolling_time_mad(jd, resid, days=200.0, min_points=10, min_days=20.0, past_only=True, add_err=None):
    """
    Rolling robust scatter: 1.4826 * median(|resid - median(resid)|) in a time window.
    Optionally add a typical photometric error in quadrature (array-like or scalar).
    """
    jd = np.asarray(jd, float)
...
def per_camera_trend_baseline(df, days_short=200.0, days_long=800.0, min_points=10, last_window_guard=120.0, t_col="JD", mag_col="mag", err_col="error",cam_col="camera#"):
    """
    ...
    Adds: baseline, resid, sigma_resid columns.
    """
